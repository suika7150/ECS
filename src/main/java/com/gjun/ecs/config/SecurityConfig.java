...
public class SecurityConfig {
        ...
        SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
                http.csrf(csrf -> csrf.disable())
                                ...
                                .exceptionHandling(
                                                exception -> exception.authenticationEntryPoint(
                                                                (request, response, authException) -> {
                                                                        response.setStatus(
                                                                                        jakarta.servlet.http.HttpServletResponse.SC_UNAUTHORIZED); // 強制回傳
                                                                                                                                                   // 401
                                ...
                                .addFilterBefore(
                                                jwtAuthenticationFilter, UsernamePasswordAuthenticationFilter.class); // 設定JWT驗證過濾器
                return http.build();
        }
 ...
 * .requestMatchers("/api/login", "/api/register", "/api/send-sms",
